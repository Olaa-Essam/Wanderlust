...
export default class DashboardView {
    ...
    storageManager;
    // stats
    aviableCountriesStateElement;
    holidaysStateElement;
    savedPlansStateElement;
    // data
    aviableCountries = [];
    publicHolidays = [];
    selectedCountry = { name: "", code: "" };
    selectedState = { name: "", lat: "", lng: "", stateCode: "" };
    selectedYear = "";
    countryDetails = null;
    // form
    countryDropdown;
    ...
    selectedDestinationCountryName;
    ...
    countryInfoPlaceholder;
    placeholderIcon;
    placeholderNormalIcon;
    placeholderErrIcon;
    placeholderNormalpragraph;
    placeholderErrpragraph;
    // clock invertal id
    ...
    findElements() {
        ...
        this.selectedDestinationFlag = document.getElementById("selected-country-flag");
        ...
        this.countryFlag = document.getElementById("country-flag"); //w-160
        ...
        this.countryLinkBtn = document.getElementById("country-link-btn");
    }
    // fill countries drop down
    fillCountriesDropDown() {
        const countryItems = this.aviableCountries.map(c => ({
            id: c.countryCode,
            name: c.name,
    ...
    async getCountryDetails(countryCode) {
        try {
            ...
            let currenciesHTML = "";
            this.countryDetails.currencies.map((c) => (currenciesHTML += `<span class="extra-tag">${c.name} (${c.code} ${c.symbol})</span>`));
            this.countryCurrencies.innerHTML = currenciesHTML;
            let languagesHTML = "";
            this.countryDetails.languages.map((l) => (languagesHTML += `<span class="extra-tag">${l}</span>`));
            this.countryLanguages.innerHTML = languagesHTML;
            let bordersHTML = "";
            if (this.countryDetails.borders && this.countryDetails.borders.length > 0) {
                this.countryDetails.borders.map((b) => (bordersHTML += `<span class="extra-tag">${b}</span>`));
            }
            else {
    ...
    getLocalTimeByTimezone(timezoneStr) {
