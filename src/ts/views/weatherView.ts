...
import type { IWeather } from "../interfaces/interfaces.js";
...
export default class WeatherView {
  ...
  // header elements
  private weatherSelection!: HTMLElement;
  private weatherSelectionCountryFlag!: HTMLImageElement;
  private weatherSelectionCountryName!: HTMLElement;
  private weatherSelectionCity!: HTMLElement;

  // content elements
  private weatherEmptyState!: HTMLElement;

  // current weather elements
  private weatherHeroCard!: HTMLElement;
  private weatherCurrnetLocationElement!: HTMLElement;
  private weatherCurrnetDateElement!: HTMLElement;
  private weatherCurrnetIconElement!: HTMLElement;
  private weatherCurrnetTempElement!: HTMLElement;
  private weatherCurrnetConditionElement!: HTMLElement;
  private weatherCurrnetFeelsElement!: HTMLElement;
  private weatherCurrnetMaxElement!: HTMLElement;
  private weatherCurrnetMinElement!: HTMLElement;

  // grid
  private weatherGridHumidityElement!: HTMLElement;
  private weatherGridHumidityPresentElement!: HTMLElement;
  private weatherGridWindElement!: HTMLElement;
  private weatherGridWindDirElement!: HTMLElement;
  private weatherGridUVElement!: HTMLElement;
  private weatherGridUVLevelElement!: HTMLElement;
  private weatherGridRainElement!: HTMLElement;
  private weatherGridRainMMElement!: HTMLElement;
  private weatherGridSunriseTimeElement!: HTMLElement;
  private weatherGridSunProgressElement!: HTMLElement;
  private weatherGridSunsetTimeElement!: HTMLElement;
  ...
  private findElements(): void {
    ...
    this.weatherSelectionCountryFlag = document.getElementById("weather-selection-country-flag")! as HTMLImageElement;
  ...
  private async loadContent(lat: string, lng: string): Promise<void>{
    try {
  ...
  private renderCurrentWeather(weather: IWeather): void {
  ...
  private renderGridWeather(weather: IWeather): void {
  ...
  private getWeatherDetails(code: number): {text: string, icon: string, bgClass: string} {
    switch (code) {
      case 0:
        return { 
          text: "Clear sky", 
  ...
  private getCurrentDateByTimezone(timezone: string): string {
    try {
      const now = new Date();
  ...
  private getWindDirectionText(degree: number): string {
  ...
  private getUvIndexDetails(uvIndex: number): {text: string, bgClass: string} {
    if (uvIndex <= 2) {
  ...
  private calculateSunProgress(currentTime: string, sunriseTime: string, sunsetTime: string): number {
